...
def tracker_eval(net, x_crop, z_crop, target_pos, target_sz, window, scale_z, p, ids, name, original_img, root_path = '/home/ly/chz/srpn_tmp'):
    cout, rout = net(z_crop, x_crop)
    delta,score = rout, cout
...
def SiamRPN_track(state, im, z_crop, ids, name):
    ...
    save_path = os.path.join('/home/ly/chz/srpn_tmp', name, '{:03d}_detection_input.jpg'.format(ids))
    cv2.imwrite(save_path, save_img)
    #print('save detection input image @ {}'.format(save_path))

    target_pos, target_sz, score = tracker_eval(net, x_crop.cuda(), z_crop.cuda(),target_pos, target_sz * scale_z, window, scale_z, p, ids, name, im)
